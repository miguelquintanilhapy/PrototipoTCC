...
namespace SAD.Services
{
    ...
    public class PdfService : IPdfService
    {
        ...
        public void GerarPdf(Proposta proposta, string caminhoArquivo)
        {
            ...
            Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.PageColor("#FFFFFF"); // sempre string
        ...
        {
            return c.Background("#1E3A5F").Padding(8);
        }

        private static IContainer DataCell(IContainer c, string bg)
        {
            return c.Background(bg).BorderBottom(1).BorderColor("#E8EDF2").Padding(8);
        }

        private static void AddResumoRow(QuestPDF.Fluent.TableDescriptor t, string label, string pct, string valor, bool bold = false, bool highlight = false)
        {
            var bg = highlight ? "#1E3A5F" : "#FFFFFF";
            var fg = highlight ? "#FFFFFF" : "#333333";
            var border = highlight ? 0 : 1;

            // Célula do label
            var cellLabel = t.Cell()
                .Background(bg)
                .BorderBottom(border)
                .BorderColor("#E8EDF2")
                .PaddingHorizontal(8)
            ...
            t.Cell()
                ...
                .Text(pct)
                .FontSize(9)
                .FontColor(highlight ? "#A8C4E0" : "#666666");

            // Célula do valor
            var cellValor = t.Cell()
                .Background(bg)
                .BorderBottom(border)
                .BorderColor("#E8EDF2")
                .AlignRight()
        ...
        private static Action<IContainer> ComposeContent(Proposta proposta) => container =>
        {
            container.PaddingTop(16).Column(col =>
            {
                ...
                col.Item().PaddingTop(6).Table(table =>
                {
                    ...
                    {
                        var bg = alternate ? "#F5F8FC" : "#FFFFFF"; // sempre string
                        alternate = !alternate;
                ...
                col.Item().PaddingTop(20).Row(row =>
                {
                    ...
                    row.ConstantItem(280).Column(resumo =>
                    {
                        ...
                        resumo.Item().PaddingTop(6).Table(t =>
                        {
                            ...
                            AddResumoRow(t, "Subtotal", "", proposta.Subtotal.ToString("C2", _ptBR));
                            AddResumoRow(t, "Overhead", $"{proposta.OverheadPercentual:0.#}%", proposta.ValorOverhead.ToString("C2", _ptBR));
                            AddResumoRow(t, "Margem de Lucro", $"{proposta.LucroPercentual:0.#}%", proposta.ValorLucro.ToString("C2", _ptBR));
                            AddResumoRow(t, "Impostos", $"{proposta.ImpostosPercentual:0.#}%", proposta.ValorImpostos.ToString("C2", _ptBR));
                            AddResumoRow(t, "VALOR TOTAL", "", proposta.TotalFinal.ToString("C2", _ptBR), bold: true, highlight: true);
                        });
                    });
                });
